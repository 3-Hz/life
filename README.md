...
**Auto camera rotation** is enabled by default and carries the lattice through a
continuous full orbit every 12 seconds. Its elevation rises and falls along a
sine wave symmetrically around the default viewing angle, within a safe range
that prevents a top-down view. Dragging or zooming takes control for a moment;
the automatic motion preserves yaw and zoom, then smoothly recenters elevation
before resuming. Turn it off in the Automaton controls when you want a
